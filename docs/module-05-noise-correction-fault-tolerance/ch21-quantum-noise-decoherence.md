...
|----------|--------|--------|--------|--------|----------|
| 振幅阻尼 | 衰减 $e^{-t/(2T_1)}$ | 衰减 $e^{-t/(2T_1)}$ | 趋向 1 | $\lvert 0\rangle$（北极） | 球体收缩到北极 |
...
| 参数 | 符号 | 典型值范围 | 说明 |
|------|------|-----------|------|
| 能量弛豫时间 | T₁ | 10–300 μs | $\lvert 1\rangle \to \lvert 0\rangle$ 寿命 |
...
因此乍看 $\tau_E \ll \tau_S$ 成立，马尔可夫近似像是好的。**但这结论有一处根本反例——超导平台固有 1/f 噪声。**

**1/f 噪声的非马尔可夫性。** 超导量子比特（Transmon 受电荷/磁通噪声主导、Fluxonium 受磁通涨落主导）普遍观测到低频（$\lesssim 1\,\mathrm{MHz}$）噪声谱 $S(\omega)\propto 1/\omega^\alpha$（$\alpha\sim 0.7\!-\!1.3$）。这类色噪声具有**长时关联**（浴关联函数拖幂律尾或低截止），不满足马尔可夫的短记忆条件。

| 噪声类型 | 时间关联 | 谱密度 | 动力学简洁性 |
|---|---|---|---|
| 白 / Ohmic 浴（本节 Kraus/Lindblad 框架） | $\langle B(t)B(0)\rangle$ 指数快速衰减 | 常数（或 Ohmic 高频截断） | Lindblad 方程（局域时间） |
| 1/f 噪声（超导平台固有） | 幂律拖尾 / 低截止 | $S(\omega)\propto 1/\omega^\alpha$ | 积分核 / 随机涨落 / 滤波函数依赖 |

**实践中的折中（而非回避）。** 主流超导实验与仿真并未因 1/f 的存在就抛弃 Kraus/Lindblad 框架，而是把 $\S1.1\!-\!\S1.10$ 当做**马尔可夫基座**，在 1/f 场景下采用以下扩展路径：

1. **准静态近似**：模拟长序列时，把 1/f 噪声的一次抽样实现视为**静态频率偏移**（$Z$ 误差），在每个 shot 中 $H_{\mathrm{noise}}$ 恒定，shot 间按 1/f 谱采样。门内动力学仍是马尔可夫的。  
2. **滤波函数噪声谱学**：用 QNS（量子噪声谱学）或 FID 衰减提取 $S(\omega)$，在做门层析 / 时序优化时识别低频权重，不需要把 1/f 压缩进 Kraus 算符。  
3. **完整的非马尔可夫模拟**（HEOM / 张量网络 / 随机过程展开）——仍处研究前沿，本章不展开。

**总结观点：** 说"马尔可夫近似足够"在超导平台中过于绝对。更准确的表述是：**本章的 Kraus/Lindblad 框架是马尔可夫 + 短时关联场景的精确语言；遇上 1/f 主导的硬件（当前超导平台常态），应在仿真层采用准静态 / 滤波函数扩展。** 近似是否成立，取决于**具体噪声谱 + 时间窗口 + 所需精度**，不能仅靠 $\tau_E\ll\tau_S$ 一锤定音。

**即时练习 1.9**

1. 用 Lindblad 主方程写出振幅阻尼信道的生成元（提示：$L = \sqrt{\gamma}\,\sigma_-$，其中 $\sigma_- = |0\rangle\langle 1|$）。  
2. 为什么长时间存储（$t$ 接近 T₁/T₂ 量级）时，马尔可夫近似可能不再准确？  
3. 以 Transmon 为例，门切片（$\sim$ ns）与空闲等待（$\sim\mu$s）这两个时间尺度上，1/f 带来的非马尔可夫性是否同等显著？为什么？  
4. （选做）搜索"滤波器函数 非马尔可夫 噪声"或"QNS quantum noise spectroscopy"——了解如何在实验中提取 $S(\omega)$ 并区分有色成分。
...
!!! tip "超导实现视角"
    **对象**：Kraus / CPTP 信道、振幅阻尼与相位阻尼、$T_1$/$T_2$、泡利与读出错误、RB 估得的平均门错误。
    **超导载体**：固定频率 Transmon（能级弛豫、纯退相）；环境 TLS、磁通/光子散粒等噪声源；色散读出链（腔 + 放大）贡献 SPAM/读出混淆；两比特门时段的可调耦合器或总线额外暴露 $T_1/T_2$ 与串扰。
    **操作要点**：$T_1$ 主导振幅阻尼（$|1\rangle\to|0\rangle$）；低频噪声与准粒子等压缩 Bloch 横向分量，进入 $T_2$/$T_\phi$；门错误常近似为短时信道或 Pauli 扭积；读出是另一类错误，需与相干错误分开建模与缓解。芯片参数 → 信道参数的桥在本章 §1.8，工程缓解见 ch20/ch23。
    **深读**：ch12 §6.6 门保真度限制（$T_1/T_2$、泄漏）；ch20 §8.2 深度–噪声、§8.3 读出差错缓解、§8.4 动力学去耦；系统缓解手法 ch23；器件与读出链 ch25、ch27。

**对照思考（选做）** 给定数据表 $T_1,T_2$ 与门时长 $t_g$，指出单比特 $R_x(\pi)$ 后更应先担心振幅阻尼还是退相；若 RB 估得的错误率远大于 $t_g/T_1$ 量级，还可能缺了哪些错误源（读出？泄漏？相干校准误差）？
