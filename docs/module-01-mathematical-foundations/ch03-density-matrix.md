...
真实量子系统几乎总是**开放系统**：它与读出腔、热浴、邻近比特等环境有相互作用，因此**单独看 $S$ 时**一般不再服从封闭的冯·诺依曼方程。

标准故事分两层：

1. **联合系统** $S+E$ 仍视为封闭，在联合 Hilbert 空间上做幺正演化（总哈密顿含耦合项）；
2. 实验往往只能操作/测量 $S$，于是对环境自由度做**部分迹**，得到约化态 $\rho_S(t)$——它通常从纯变混，这就是退相干的密度矩阵语言。

联合空间用张量积构造（§1.8），“扔掉环境”用偏迹（§1.9.2）。二者在 **§3.4 约化密度矩阵**讲清之后，于 **§3.4.7** 再写明 $H_{SE}$ 拆分与

$$
\rho_S(t)=\mathrm{Tr}_E\!\big[U_{SE}(t)(\rho_S(0)\otimes\rho_E(0))U_{SE}^\dagger(t)\big]
$$

的来历。Kraus / Lindblad / 噪声信道的系统展开见模块五 ch21。
...
### 3.4.7 开放系统演化：$H_{SE}$ 与偏迹

§3.3.4 只预告了“联合封闭、系统约化”。现在偏迹已定义，可以把开放演化写成可计算的三步。

**1. 联合空间（接 §1.8）**  
系统 $S$ 与环境 $E$ 的态空间是张量积 $\mathcal{H}_S\otimes\mathcal{H}_E$。乘积初态写作 $\rho_S(0)\otimes\rho_E(0)$（$S$ 与 $E$ 尚未关联）；更一般的 $\rho_{SE}(0)$ 也允许，但入门默认乘积。
...
- $H_S\otimes I_E$：只转系统，环境当旁观者；  
- $I_S\otimes H_E$：只转环境；  
- $H_{\mathrm{int}}$：**耦合**，真正把 $S$ 与 $E$ 缠在一起。它必须写在联合空间上，典型结构是若干项 $A_S\otimes B_E$（局域算符的张量积再相加），而不是两个小空间里各写一个矩阵再“假装相加”——后者是直和图像，不内建关联（§1.8.2 对照）。

极简示意（两比特玩具环境）：令 $E$ 也是一个量子比特，$H_{\mathrm{int}}=g\,Z_S\otimes X_E$。$g=0$ 时联合演化因子化，约化后 $S$ 仍幺正；$g\neq 0$ 时 $S$ 与 $E$ 可纠缠，只看 $S$ 会丢纯度。

**3. 联合幺正 + 对环境偏迹（接 §1.9.2）**  
$H_{SE}$ 不显含时，

$$
U_{SE}(t)=e^{-i H_{SE}t/\hbar},
\quad
\rho_{SE}(t)
= U_{SE}(t)\,\rho_{SE}(0)\,U_{SE}^\dagger(t).
$$
...
$$
\rho_S(t)
= \mathrm{Tr}_E\big[\rho_{SE}(t)\big]
= \mathrm{Tr}_E\!\Big[
U_{SE}(t)\big(\rho_S(0)\otimes\rho_E(0)\big)U_{SE}^\dagger(t)
\Big].
$$

这就是 §3.3.4 预告的公式：**先在 $\otimes$ 空间里耦合演化，再 $\mathrm{Tr}_E$ 丢掉环境**。  
$\rho_S(t)$ 对 $t$ 的映射一般是完全正、保迹的**量子信道**，不必再是某个 $H_S$ 生成的幺正共轭；纯度 $\mathrm{Tr}(\rho_S^2)$ 可随时间下降——退相干。

**操作清单（会写，不要求解一般 $H_{\mathrm{int}}$）：**

1. 标明 $S$、$E$ 与 $\mathcal{H}_S\otimes\mathcal{H}_E$；  
2. 写出 $H_{SE}=H_S+H_E+H_{\mathrm{int}}$（张量提升）；  
3. 选初态（常 $\rho_S\otimes\rho_E$）；  
4. 形式上写 $U_{SE}$ 与 $\rho_{SE}(t)$；  
5. 做 $\mathrm{Tr}_E$ 得 $\rho_S(t)$；解释纯度/相干如何变。

完整 Kraus 表示、Lindblad 主方程与实验噪声信道见 ch21；硬件上的 $T_1/T_2$、读出腔耦合见模块六。
...
密度矩阵能描述制备、噪声后的输出、子系统约化结果——于是同一课里会反复出现**两个态** $\rho$ 与 $\sigma$。若没有定量比较，就无法回答工程与理论中的硬问题，例如：

- 门/制备是否“够好”：目标态 $\rho_{\mathrm{ideal}}$ 与实验层析得到的 $\rho_{\mathrm{exp}}$ 差多少？  
- 噪声伤了多少：信道输入 $\rho$ 与输出 $\mathcal{E}(\rho)$ 偏离多大？  
- 算法输出是否可信：理想终点与含噪终点能否被测量分辨？  
- 纠错与阈值论证：逻辑错误把码字推到多远？

距离度量把“有多像 / 多容易分清”收成 $[0,1]$ 上的数，并带单调性等定理，才能写进标定报告与误差界。本章主线两个：**保真度**（重叠、相似）与**迹距离**（最优区分、统计可分性）。二者互补，不是重复定义。

### 3.6.1 保真度

**物理内核：** 保真度回答“两个态作为量子信息载体有多重叠”。纯态时它就是 $|\langle\psi|\phi\rangle|$——同一射线则 $1$，正交则 $0$；混态时用 Uhlmann 图像看成“最好纯化之间的重叠”（见下节）。

**作用 / 何时用：**

- 过程与态的**质量指标**（态保真、门保真的理论原型）；  
- 数据处理：对同一 CPTP 信道，$\mathrm{F}(\mathcal{E}(\rho),\mathcal{E}(\sigma))\ge \mathrm{F}(\rho,\sigma)$（保真度不减，见 Uhlmann 推论）；  
- 适合写“有多接近目标”，不直接等于“实验上一次测量猜对的概率”。
...
取值 $0 \leq F(\rho, \sigma) \leq 1$。

- $F(\rho, \sigma) = 1$：$\rho = \sigma$（相同态）
- $F(\rho, \sigma) = 0$：支撑正交（完全不重叠）
...
> **Uhlmann 定理的关键推论（数据处理）**：对任意 CPTP 映射 $\mathcal{E}$，保真度**不减**——$F(\mathcal{E}(\rho), \mathcal{E}(\sigma)) \geq F(\rho, \sigma)$。同一噪声作用后两态不会“更不像”；与之对照，迹距离**不增** $D(\mathcal{E}(\rho),\mathcal{E}(\sigma))\le D(\rho,\sigma)$（可区分性不升）。二者一起刻画信息损失。

### 3.6.3 迹距离

**物理内核：** 迹距离回答“若允许做最优测量，两个态的统计有多容易被分清”。它是经典总变差距离在量子态上的提升：先看 $\rho-\sigma$ 的奇异值，再归一到 $[0,1]$。

**作用 / 何时用：**

- **假设检验 / 态区分**：等先验下，最优猜对概率为 $\frac12\big(1+D(\rho,\sigma)\big)$；  
- **误差传播与钻石范数界的底色**：电路误差、模拟误差常最终落到输出态的迹距离；  
- 适合写“对手或实验者最多能区分到什么程度”，而不是“和目标有多像”的单一分数。
...
- $D(\rho, \sigma) = 1$：存在测量可完美区分

**测量操作含义**：对任意 POVM $\{E_m\}$，两态输出分布的总变差不超过 $D$；且存在测量达到该上界：
...
前面已分别给出 $F$ 与 $D$ 的内核。对照选型：

| 度量 | 内核一句话 | 典型用途 | 纯态–纯态 | 纯态–$I/2$ |
|------|------------|----------|-----------|------------|
| 保真度 $F$ | 重叠 / 有多像 | 标定“靠近目标”、过程质量 | $|\langle\psi|\phi\rangle|$ | $1/\sqrt{2}$ |
| 迹距离 $D$ | 最优可区分程度 | 误差界、区分任务、输出可分辨性 | $\sqrt{1-|\langle\psi|\phi\rangle|^2}$ | $1/2$ |

经验口诀：**报“像不像目标”优先 $F$；报“差到会不会被测出来 / 误差有多凶”优先 $D$**。二者由 §3.6.4 不等式锁在一起，可互相粗估。

**例 3.43（量子存储器）** 初始 $\rho=|0\rangle\langle0|$。退相干后
$\sigma=(1-p)|0\rangle\langle0|+p|1\rangle\langle1|$。

$$
F(\rho,\sigma)=\sqrt{\langle0|\sigma|0\rangle}=\sqrt{1-p},
\quad
D(\rho,\sigma)=p.
$$

$p=0$：$F=1,D=0$（完好）。$p=1/2$：$F=1/\sqrt{2},\,D=1/2$（与例 3.40 同类）。$p$ 上升时 $F$ 下降、$D$ 上升——同一物理过程的两种读数。
...
| 开放系统 / 系统–环境 | Open system / system–environment | 3.3.4, 3.4.7 |
...
> 4. **约化密度矩阵** $\rho_A=\mathrm{Tr}_B(\rho_{AB})$；开放系统先在 $H_{SE}=H_S+H_E+H_{\mathrm{int}}$ 下联合幺正，再 $\mathrm{Tr}_E$ 得 $\rho_S(t)$（§3.4.7）。
> 5. **纠缠与熵**：两体纯态纠缠熵 $S(\rho_A)=S(\rho_B)$；Bell 态为 $1$ bit。
> 6. **距离**：$F$ 表重叠（标定“像不像”），$D$ 表最优可区分性（误差/区分）；$1-F\le D\le\sqrt{1-F^2}$。
>
> 密度矩阵是处理混合态与开放系统的工作语言；噪声信道与 Lindblad 见模块五。
