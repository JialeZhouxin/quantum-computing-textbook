...
$$
U = R_z(\phi) R_x(\theta) R_z(\psi)
$$

其中 $\phi, \theta, \psi$ 是三个实参数。
...
在数学上，$R_z(\theta)$ 是一个对角矩阵——它给 $|0\rangle$ 和 $|1\rangle$ 加上相反的相位：

$$
R_z(\theta) = \begin{pmatrix} e^{-i\theta/2} & 0 \\ 0 & e^{i\theta/2} \end{pmatrix}
$$

在实际的量子计算机中，执行 $R_z(\theta)$ 的最常见方式是**不执行任何物理操作**——而是通过"虚拟"的方式实现。

**虚拟 $Z$ 门的核心思想**：

> 如果系统处于 $|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$，我们要施加 $R_z(\theta)$。与其真的改变量子态，不如在**后续所有门操作中"记住"我们绕 $z$ 轴旋转了 $\theta$ 角**。

具体做法：在软件层面维护一个**相位帧（phase frame）**——一个"旋转的坐标系"。所有后续的微波脉冲相对于这个旋转坐标系来定义。

**为什么这有效？**

考虑一个物理 $X$ 脉冲（绕 $x$ 轴旋转 $\pi$）。在实验室坐标系中，这个脉冲沿 $x$ 轴方向。但如果我们在施加脉冲之前"虚拟地"绕 $z$ 轴旋转了一个角度 $\theta$，那么同样的物理脉冲在旋转后的坐标系中相当于绕 $x$ 轴旋转——只是 $x$ 轴的定义变了。

数学上，相位帧跟踪等价于：

1. 在软件中记录一个累计相位 $\phi_{\text{frame}}$
2. 对所有后续的微波脉冲，将脉冲的相位偏移 $\phi_{\text{frame}}$
3. 更新 $\phi_{\text{frame}}$ 为 $\phi_{\text{frame}} + \theta$

**虚拟 $Z$ 门的优势**：

1. **零时间**：不消耗任何门时间，因为它不是物理操作
2. **零误差**：没有硬件误差，纯粹是软件层面的跟踪
3. **高保真度**：因为不引入任何噪声
4. **任意角度**：$R_z(\theta)$ 对任意 $\theta$ 都是等价的——没有离散化误差

**例 1.10** 假设量子计算机的初始相位帧为 $\phi = 0$。我们执行以下操作：

1. 施加物理 $R_x(\pi/2)$ 脉冲
2. 虚拟 $R_z(\pi/4)$：将相位帧更新为 $\phi = \pi/4$
3. 施加物理 $R_x(\pi/2)$ 脉冲（相对于当前相位帧）

第三步中的物理脉冲在实验室坐标系中实际上是绕 $x$ 轴旋转 $\pi/2$，但相对于相位帧，它等效于 $R_x(\pi/2)$ 作用在已经被 $R_z(\pi/4)$ 旋转过的态上。

最终效果：$$U = R_x(\pi/2) R_z(\pi/4) R_x(\pi/2)$$（忽略全局相位）。

虚拟 $Z$ 门是现代量子控制中最重要的"免费午餐"之一。几乎所有超导量子计算平台都使用相位帧跟踪来实现 $Z$ 门。
